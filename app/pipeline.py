...
from app.storage import STAGE_CSV_SCHEMAS, append_record

DEFAULT_MODEL = "openai/gpt-4o-mini"


def _extract_stage_rows(stage: Stage, out: dict) -> list[dict]:
    headers = STAGE_CSV_SCHEMAS[stage.value]
    created_at = datetime.now(timezone.utc).isoformat()
    base_reason = str((out.get("final_decider_output") or {}).get("reasoning") or "Generated by decider")

    if stage == Stage.ELEMENT_GENERATION:
        items = ((out.get("final_decider_output") or {}).get("final_output") or {}).get("final_selected_elements") or []
        rows = []
        for item in items[:20]:
            rows.append({
                "element_type": item.get("element_type", "general"),
                "name": item.get("name") or item.get("title") or "untitled",
                "description": item.get("description") or item.get("text") or json.dumps(item)[:800],
                "reasoning_for_choosing": item.get("reasoning") or base_reason,
                "created_at": created_at,
            })
        if rows:
            return rows

    payload = json.dumps((out.get("final_decider_output") or {}).get("final_output") or out.get("final_decider_output") or out)[:3000]
    row = {"created_at": created_at, "reasoning_for_choosing": base_reason}
    for key in headers:
        if key in {"id", "created_at", "reasoning_for_choosing"}:
            continue
        if key in {"name", "title", "headline", "hook", "video_headline", "caption"}:
            row[key] = f"{stage.value.replace('_', ' ')} output"
        else:
            row[key] = payload
    return [row]
    ...
    async def execute(self, run: RunState) -> RunState:
        ...
        try:
            for stage in run.config.selected_stages:
                ...
                pending_rows = _extract_stage_rows(stage, out)
                run.pending_updates[stage.value] = {
                    "headers": STAGE_CSV_SCHEMAS[stage.value],
                    "rows": pending_rows,
                    "status": "pending_confirmation",
                    "csv_key": f"data/{stage.value}.csv",
                }
                self._log(run.run_id, stage.value, "storage", "save_update", f"pending rows={len(pending_rows)} awaiting confirmation")
